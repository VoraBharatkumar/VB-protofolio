...
export function Contact() {
  ...
  const handleMethodSelect = async (method: 'email' | 'whatsapp') => {
    ...
    if (method === 'whatsapp') {
      ...
      const whatsappURL = `https://wa.me/919725454382?text=${whatsappMessage}`
      ...
      try {
        // Step 1: Get the Web3Forms access key securely from our API route
        ...
        //  server-side requests receive HTTP 403 Cloudflare challenge)
        ...
        console.log('Web3Forms raw response:', {
          status: web3Response.status,
          statusText: web3Response.statusText,
          headers: Object.fromEntries(web3Response.headers.entries()),
          body: responseText,
        })
        ...
        console.log('Web3Forms parsed result:', web3Result)

        // Only show success when Web3Forms itself confirms successful submission
        if (web3Response.ok && web3Result.success) {
          setSubmitStatus('success')
          setShowModal(false)
          document.querySelector('form')?.reset()
        } else if (!web3Response.ok) {
          ...
          console.error('Web3Forms returned unsuccessful response:', web3Result)
          setSubmitStatus('error')
        }
      } catch (error) {
              ...
              <div className="lg:col-span-5">
                ...
                <Reveal delay={0.16}>
                  <div className="mt-8 space-y-4 border-t border-[#E2E8F0] pt-6">
                    ...
                    >
                      <motion.span 
                        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E3A8A] group-hover:bg-[#1E3A8A] group-hover:text-white transition-colors"
                    ...
                    <div className="flex items-center gap-3 text-xs sm:text-sm text-[#64748B]">
                      <motion.span 
                        className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E3A8A]"
                        whileHover={{ scale: 1.1, rotate: -5 }}
                      >
                ...
                <Reveal delay={0.12}>
                  ...
                  >
                    ...
                    <motion.button
                      ...
                      className="group relative inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#2563EB] px-5 py-3.5 text-xs font-bold uppercase tracking-wider text-white shadow-xs transition-all duration-200 hover:bg-[#1D4ED8] disabled:opacity-70 disabled:cursor-not-allowed"
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-md rounded-2xl bg-white border border-[#E2E8F0] p-6 sm:p-8 shadow-xl"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg sm:text-xl font-bold text-[#1E3A8A]">Choose Submission Method</h3>
                <button
                  onClick={() => setShowModal(false)}
              ...
              <div className="space-y-3">
                <motion.button
                  ...
                  disabled={isSubmitting}
                  className="w-full flex items-center gap-4 rounded-xl border-2 border-[#E2E8F0] p-4 sm:p-5 hover:border-[#2563EB] hover:bg-blue-50/50 transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-[#2563EB]/10 text-[#2563EB] group-hover:bg-[#2563EB] group-hover:text-white transition-colors">
                    <Mail className="h-6 w-6" />
                  </div>
                  <div className="flex-1 text-left">
                    <div className="text-sm sm:text-base font-bold text-[#1E3A8A]">Send via Email</div>
                    <div className="text-xs text-[#64748B] mt-0.5">Direct delivery to our executive inbox</div>
                  </div>
                  {isSubmitting && (
                    <motion.div
                      animate={{ rotate: 360 }}
                      transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                      className="h-5 w-5 border-2 border-[#2563EB] border-t-transparent rounded-full"
                    />
                  )}
                </motion.button>
                ...
                <motion.button
                  ...
                  disabled={isSubmitting}
                  className="w-full flex items-center gap-4 rounded-xl border-2 border-[#E2E8F0] p-4 sm:p-5 hover:border-green-500 hover:bg-green-50/50 transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-green-500/10 text-green-600 group-hover:bg-green-500 group-hover:text-white transition-colors">
                    <MessageSquare className="h-6 w-6" />
                  </div>
                  <div className="flex-1 text-left">
                    <div className="text-sm sm:text-base font-bold text-[#1E3A8A]">Send via WhatsApp</div>
                    <div className="text-xs text-[#64748B] mt-0.5">Instant message to +91 9725454382</div>
                  </div>
                  {isSubmitting && (
                    <motion.div
                      animate={{ rotate: 360 }}
                      transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
                      className="h-5 w-5 border-2 border-green-600 border-t-transparent rounded-full"
                    />
                  )}
                </motion.button>
              </div>
              ...
              <button
                onClick={() => setShowModal(false)}
                className="mt-4 w-full rounded-xl border border-[#E2E8F0] px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-[#64748B] hover:bg-gray-50 transition-colors"
              >
                Cancel
              </button>
            </motion.div>
